...
class AIAnalysisThread(QThread):
    ...
    def __init__(self, prompt: str, settings: dict, parent=None):
        super().__init__(parent)
        self.prompt = prompt
...
class AIAssistantTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.ai_settings_btn = QPushButton("⚙️")
        self.ai_settings_btn.setToolTip("Configure AI")
    ...
    def update_theme(self):
        ...
        self.ai_settings_btn.setIcon(create_themed_icon("icons/gear.svg", text_color))
        self.send_button.setIcon(create_themed_icon("icons/paper-airplane.svg", text_color))
        
    def _populate_prompts(self):
        for category, prompts in self.ai_prompts.items():
            cat_item = QTreeWidgetItem(self.prompt_tree, [category])
    ...
    def send_message(self):
        ...
        self.start_ai_analysis(text)
        
    def _add_chat_bubble(self, message: str, is_user: bool):
    ...
    def start_ai_analysis(self, prompt: str):
        settings = self.parent.get_ai_settings()
        if not settings:
            return
        self.typing_indicator.show()
        self.ai_thread = AIAnalysisThread(prompt, settings, self)
    ...
    def handle_ai_response(self, chunk: str, is_thinking: bool, is_final: bool):
        self.typing_indicator.hide()
        if is_thinking:
            if not self.thinking_widget:
                item = QListWidgetItem(self.chat_list)
                self.thinking_widget = QLabel("Thinking…")
                self.chat_list.addItem(item)
                self.chat_list.setItemWidget(item, self.thinking_widget)
                item.setSizeHint(self.thinking_widget.sizeHint())
            self.thinking_widget.setText(self.thinking_widget.text() + chunk)
            return
